...
namespace MATRIX { 			//We want to define our own namespace for this library 
	...
	{
		arr.reserve(ROWS*COLS);
		for(size_t i=0;i<M;++i)for(size_t j=0;j<N;++j)this->arr[i][j]=0; 
		//T copyArr [M*N]= {0}; 
	...
	{
		arr.reserve(M); 
		for(size_t i=0;i<M;++i)arr[i].reserve(N); 
		arr.reserve(ROWS*COLS); 
		for(size_t i=0;i<M;++i)
	...
	Matrix<M, N, T>:: Matrix(const std::vector<T> vec)
	{
		...
		{
			for(size_t i=0;i<N;++i)v.push_back(vec[index++]); 
...
/*	template<size_t M, size_t N, typename T> 
	...
	bool Matrix<M,N,T>:: operator==(const std::vector<T>& dataVec) const
	{
		...
		for(size_t i=0;i<M;++i)
			for(size_t j=0;j<N;++j)
			{
				if(flag)if(fabs(this->arr[i][j]-dataVec[vecIndex])>=EPSILON)return false; 
				if(!flag)if(this->arr[i][j]!=dataVec[vecIndex])return false; 
				vecIndex++; 
			}
		return true; 
	}
	...
	bool Matrix<M,N,T>:: operator==(const std::vector<U>& dataVec) const
	{
		...
		for(size_t i=0;i<M;++i)
			for(size_t j=0;j<N;++j)
			{
				if(flag && fabs(this->arr[i][j]-dataVec[vecIndex])>=EPSILON)return false;
	...
	{
		for(size_t i=0;i<M;++i)for(size_t j=0;j<N;++j)this->arr[i][j]=toCopy.arr[i][j];
	...
	void Matrix<M, N, T>:: multiply( const Matrix<M,P,T>& m1, const Matrix<P,N,T>& m2)
	{
		...
		for(size_t i=0;i<M;++i)
			for(size_t j=0;j<N;++j)
			{
				...
				{
					if(m1.arr[i][k]!=0 && m2.arr[k][j]!=0)currEntry+=m1.arr[i][k]*m2.arr[k][j]; 
				}
	...
	void Matrix<M, N, T>:: multiply( const Matrix<M,P,T>& m1, const Matrix<P,N,U>& m2)
	{
		...
		for(size_t i=0;i<M;++i)
			...
			{	
				...
				for(size_t k=0;k<P;++k)
				{
					if(m1.arr[i][k]!=0 && m2.arr[k][j]!=0)currEntry+=static_cast<T>(m1.arr[i][k]*m2.arr[k][j]); 
				}
				this->arr[i][j]=currEntry; 
			}
	}  
